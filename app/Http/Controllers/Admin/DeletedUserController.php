...
class DeletedUserController extends Controller
{
    public function index()
    {
        try {
            if (request()->ajax()) {
                return datatables()->of(User::with('profile')->whereNull('closed_on')->where('role_id', 2)->onlyTrashed()->get())
                    ->addIndexColumn()
                    ...
                    ->addColumn('action', function ($data) {
                        return '<button title="Retrieve" type="button" name="retrieve" id="' . $data->id . '" class="delete btn btn-success btn-sm"><i class="fa fa-recycle"></i></button>';
                    })->rawColumns(['profile_picture', 'action'])->make(true);
            }
        } catch (\Exception $ex) {
            return redirect('/')->with('error', $ex->getMessage());
        }
        return view('admin.deleted-user.list');
    }

    final public function retrieve($id)
    {
        $content=User::where('id', $id)->onlyTrashed()->first();
